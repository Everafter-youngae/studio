...
- `assets/` — `hero-tulip.png`, `ocean.jpeg`, `pond.jpeg`, `hero.mp4`, `voice-sample-1.m4a`, `og-card.jpg`
...
- **Progress label** (`index.html`): `IntersectionObserver` over `[data-progress]` sections updates `#progressCopy`.
...
- **Hero video**: injects a muted looping `<video>` over the hero image on wide viewports, skipped under `prefers-reduced-motion`, falling back to the still image if playback is blocked. The `src` carries a `?v=N` cache-buster — **bump it whenever `assets/hero.mp4` is replaced**, or browsers keep the old file.
- **Voice sample** (`index.html`): custom transport for `#voiceAudio` (`assets/voice-sample-1.m4a`) — play/pause, a `<input type=range>` scrubber overlaid transparently on a visible progress bar, and elapsed/total clocks.
...
- **Every photo in `assets/` is portrait** (`hero-tulip.png` 564×702, `ocean.jpeg` 0.46, `pond.jpeg` 0.58) while `og:image` is cropped to **1.91:1** by KakaoTalk and Twitter. Pointing `og:image` straight at one of them yields a thin band through the middle. `assets/og-card.jpg` (1200×630) is the pattern to follow: the portrait photo gets a vertical panel on the left, brand line and headline on the right, nothing cropped. It was generated by rendering an HTML layout at 1200×630 with Playwright (`page.locator('.og').screenshot({type:'jpeg', quality:86})` → 41KB). Check any image's ratio with `sips -g pixelWidth -g pixelHeight <file>`; there is no ImageMagick here.
- Only `card/` uses `og-card.jpg` so far; `index.html`, `ask.html` and `review.html` still point at portrait images and have the same cropping problem.
